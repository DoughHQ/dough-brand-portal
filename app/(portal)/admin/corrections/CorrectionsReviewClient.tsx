  ...
  if (photoOnly) {
    return {
      label: row.evidence_image_url
        ? 'Photo only — extract or enter values to apply (Override, coming next)'
...
export default function CorrectionsReviewClient({ initialRows }: Props) {
  ...
  const handleAction = useCallback(
    async (submissionId: string, action: 'approve' | 'reject', notes?: string | null) => {
      const row = rows.find((r) => r.id === submissionId)
      if (!row) return

      if (action === 'approve' && !canApproveAsIs(row)) {
        setError(
      ...
      setError(null)
      const result = await reviewCorrectionAction(submissionId, action, notes)
      setBusyId(null)
      if (!result.ok) {
      ...
      setFlash(action === 'approve' ? 'Approved and applied.' : 'Rejected.')
    },
    [rows]
  )
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === 'a' || e.key === 'A') {
        e.preventDefault()
        if (canApproveAsIs(focused)) void handleAction(focused.id, 'approve')
        else {
          setError(
            'Approve disabled for photo-only corrections — extract/enter values (override) or reject.'
          )
        }
        return
      }
      if (e.key === 'r' || e.key === 'R') {
        ...
        void handleAction(focused.id, 'reject', chip)
        return
      }
      if (e.key === 'Escape') {
        setExpandedImg(null)
  ...
  }, [busyId, focused, handleAction, rejectChipById, rows.length])
    ...
    <div style={{ fontFamily: 'var(--font-sans)', maxWidth: 1100, margin: '0 auto', padding: '36px 32px 96px' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--ink-50)', lineHeight: 1.55, maxWidth: 640 }}>
          Evidence first. Structured proposals can be approved as-is.
          Nutrition and ingredients are photo-only — extract or enter values to apply (override), or reject.
        </p>
        <p style={{ fontSize: 12, color: 'var(--ink-30)', marginTop: 10 }}>
          Keyboard: <kbd style={kbdStyle}>J</kbd>/<kbd style={kbdStyle}>K</kbd> move ·{' '}
          <kbd style={kbdStyle}>A</kbd> approve · <kbd style={kbdStyle}>R</kbd> reject (with chip)
        </p>
      </div>
              ...
              >
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--sage)', marginBottom: 4 }}>
                      ...
                      {suggestion.photoOnly ? ' · photo-only' : ''}
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
                  <div style={{ padding: '18px 20px', borderRight: '1px solid var(--ink-10)' }}>
                    <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-30)', marginBottom: 8 }}>
                      Current
                    </div>
                  ...
                  <div style={{ padding: '18px 20px' }}>
                    ...
                    {row.claude_reasoning && (
                      <div style={{ fontSize: 12, color: 'var(--ink-30)', marginTop: 10, lineHeight: 1.45, fontStyle: 'italic' }}>
                        Prior automated review (retired): {row.claude_reasoning}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--r-sm)',
                      border: 'none',
                      background: !approveOk || busy ? 'var(--ink-10)' : 'var(--sage)',
                      color: !approveOk || busy ? 'var(--ink-30)' : 'white',
                      fontSize: 13,
                      fontWeight: 500,
                      cursor: !approveOk || busy ? 'not-allowed' : 'pointer',
                      fontFamily: 'var(--font-sans)',
                    }}
                  >
                    {busy && busyId === row.id ? 'Working…' : approveOk ? 'Approve' : 'Approve unavailable'}
                  </button>
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      ...
                      void handleAction(row.id, 'reject', chip)
                    }}
                    style={{
                      flex: 1,
                      padding: '11px 16px',
